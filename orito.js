    ...
    async function handleUserSendMessage() {
        ...
        if (matchedProds.length > 0) {
            ...
            matchedProds.forEach(p => {
                ragContext += `- [${p.name}](detail:${p.id}) | Category: ${p.category} | Brand: ${p.brand} | Est. Price: ₹${p.price.toLocaleString('en-IN')} | Catalogue Page Ref: Page ${p.page}\n  Specs: ${JSON.stringify(p.specs)}\n  Description: ${p.description}\n`;
            });
        } else {
...
- Do not make up product details or prices. Only use facts in the provided context or ask them to contact the sales desk.
...
- Provide the catalog page reference and estimate price whenever relevant.
- Estimate price is in INR (₹).
